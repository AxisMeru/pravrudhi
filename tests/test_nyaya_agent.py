...
def _est(fid: str, text: str, quote: str, p: float = 0.97) -> ElementJudgment:
    ...
    return ElementJudgment("established", p, fid, quote, start, start + len(quote))
...
class TestLoop:
    def test_proof_when_every_element_is_established_with_a_valid_quote(self, tmp_path: Path) -> None:
    ...
    def test_first_attempt_uses_the_training_statute_retry_the_retrieved_one(self, tmp_path: Path) -> None:
        script = _proof_script(TOY_FACTS)
        bad = ElementJudgment("established", 0.97, "F2", "never to marry", 0, 5)  # wrong offsets
        script[BNS69_EL[0]] = [bad, script[BNS69_EL[0]][0]]
        run, judge, _ = _run(tmp_path, script)
        el0 = [r for r in judge.requests if r.element == BNS69_EL[0]]
        assert [r.statute for r in el0] == ["TRAINING statute text for bns69", "RETRIEVED statute text for bns69"]
    ...
    def test_invalid_quote_after_retries_is_not_established_never_repaired(self, tmp_path: Path) -> None:
        script = _proof_script(TOY_FACTS)
        script[BNS69_EL[1]] = [ElementJudgment("established", 0.99, "F3", "Lata slept with Kiran", 0, 21)]
        run, judge, registry = _run(tmp_path, script)
        c = run.contracts[0]
        el1 = c.elements[1]
        assert el1.status == "not_established"
        assert el1.quote_check == "quote_mismatch"
    ...
    def test_retry_budget_is_config(self, tmp_path: Path) -> None:
        script = _proof_script(TOY_FACTS)
        script[BNS69_EL[1]] = [ElementJudgment("established", 0.99, "F9", "x", 0, 1)]
    ...
    def test_denial_claimed_but_unquotable_refers_rather_than_proves(self, tmp_path: Path) -> None:
        ...
        script = _proof_script(TOY_FACTS)
        script[BNS69_DENY] = [ElementJudgment("established", 0.95, "F3", "invented words", 0, 14)]
...
class TestAudit:
    def test_every_step_is_a_jsonl_line_with_hashed_inputs(self, tmp_path: Path) -> None:
        ...
        for s in ("ingest", "select_contracts", "describe", "judge", "quote_check", "assemble", "lean_check", "outcome"):
            assert s in steps
        ...
        # Inputs are hashed, never copied: no fact's text appears in any line's inputs.
        assert not any(TOY_FACTS[0] in json.dumps(x.get("inputs_sha256")) for x in lines)
