"""VLLMDecoder primary -> fallback rules must match `nyaya_judges.HouseJudge`'s exactly (T1, closing the
fallback gap flagged at handoff -- see test_nyaya_judge_fallback.py, which this mirrors): only a transport
failure, a 5xx or a 429 moves to the next backend; a 4xx surfaces; the bearer key goes to the primary only;
`backend_index` records which backend answered, and `TypedHouseJudge` carries it through as `backend_used`,
same as `HouseJudge`.
"""
...
def _completion() -> bytes:
    return json.dumps(
        {"model": "m", "choices": [{"text": " established F1", "finish_reason": "length", "logprobs": {"top_logprobs": [_TOP]}}]}
    ).encode()
...
def _judge(monkeypatch: pytest.MonkeyPatch, net: _Net) -> TypedHouseJudge:
    monkeypatch.setattr("urllib.request.urlopen", net)
    decoder = VLLMDecoder(
        base_url=PRIMARY, model="nyaya-judge-4b", timeout_s=5, api_key="primary-secret", fallback_urls=[FALLBACK]
    )
    return TypedHouseJudge(tau=0.5, statute_chars=600, decoder=decoder, max_tokens=30, top_logprobs=20)


def _req() -> Any:
    from pravrudhi.application.nyaya_judges import JudgeRequest

    return JudgeRequest("c", "an element", False, "statute", "narrative", (("F1", "a fact"),))
