...
gold set only. Scoring belongs where the verifier lives (`prabhasa-nyaya`'s Lean `Verdict.of`), so that a gold
set which grades its own grader is impossible by construction -- the labels here are derived from the world
independently of any verifier's opinion about it, and `satpratipaksa`/`badhita` are labelled as not decided by
that verifier rather than scored against it.
...
def _satpratipaksa_bank(pool: list[dict[str, Any]], per_class: int) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
...
def build_gold_set(per_class: int, seed: int = 0) -> list[dict[str, Any]]:
    ...
    # 1x for the valid bank itself, 2x per satpratipaksa item (base + counter), 1x per badhita item (base).
