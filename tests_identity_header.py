...
def test_query_token_still_works_with_custom_header(monkeypatch: pytest.MonkeyPatch) -> None:
    """?access_token query param should still fallback to Authorization when custom header is set."""
    pytest.importorskip("jwt")
    secret = "test-secret"
    monkeypatch.setenv("PRAVRUDHI_AUTH", "optional")
    monkeypatch.setenv("SUPABASE_URL", "https://example.supabase.co")
    ...
    # ?access_token should still work (it creates Authorization header internally)
    resp = client.get(f"/whoami?access_token={token}")
    assert resp.status_code == 200
    # When custom header is set, Authorization (even from ?access_token) should be ignored
    assert resp.json()["user"] is None

    # But if the custom header is provided, it should take precedence
    resp = client.get(
        f"/whoami?access_token=dummy",
